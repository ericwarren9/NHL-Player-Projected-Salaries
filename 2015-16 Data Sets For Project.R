# PURPOSE: To explore some of the data we have with the NHL Player Stats and Their Salary For 2015-16 Season
...
tempData <- tibble()
for (i in 1:31) {
  url <- paste0("https://www.capfriendly.com/browse/active/2016?stats-season=2022&display=caphit-percent,skater-individual-advanced-stats,skater-on-ice-advanced-stats,goalie-advanced-stats,type&hide=team,clauses,handed,salary&pg=", i)
  ...
  salaryCF15 <- bind_rows(salaryCF15, tempData)
}

# Get column names for data that need to be numeric but currently are character
numericValues  <- salaryCF15 %>%
  select(-c("PLAYER", "POS", "TOI", "TYPE", "EXPIRY", "CAP HIT", "CAP HIT %")) %>%
  colnames()


salaryCF15[, numericValues] <- lapply(salaryCF15[, numericValues],function(x){as.numeric(gsub(",", "", x))})

# Convert the 'CAP HIT' to a numeric value
salaryCF15$`CAP HIT` <- as.numeric(gsub('[$,]', '', salaryCF15$`CAP HIT`))

# Convert the 'CAP HIT %' to a numeric value
salaryCF15$`CAP HIT %` <- as.numeric(gsub("%", "", salaryCF15$`CAP HIT %`))

# Change all percentages into decimals
percentageValues <- salaryCF15 %>%
  select(c("SF%", "CF%", "FF%", "xGF%", "CAP HIT %")) %>%
  colnames()

salaryCF15[, percentageValues] <- lapply(salaryCF15[, percentageValues],function(x){x / 150})

# Convert the time on ice to seconds of game play
library(lubridate)
salaryCF15$TOI <- substr(salaryCF15$TOI, start = 1, stop = 5)
time15 <- ms(salaryCF15$TOI)
salaryCF15$TOI <- time15@minute * 60 + time15@.Data

# Separate player name and Rank to use just player name
salaryCF15 <- salaryCF15 %>%
...
library(stringi)
salaryCF15$PLAYER <- stri_trans_general(salaryCF15$PLAYER, id = "Latin-ASCII")
...
salaryCF15$POS <- sapply(salaryCF15$POS, firstPosition)
salaryCF15$POS <- sapply(salaryCF15$POS, realPosition)
...
salaryMPPlayer15 <- read_csv("RawData/skatersMP2015-16.csv")

# Get rid of not needed variables
salaryMPPlayer15 <- salaryMPPlayer15  %>%
...
nhl_onIceStats  <- salaryMPPlayer15 %>%
  select(PLAYER,
         position,
         games_played,
         situation,
...
nhl_offIceStats  <- salaryMPPlayer15 %>%
  select(PLAYER,
         position,
         games_played,
         situation,
...
nhl_onIceFStats  <- salaryMPPlayer15 %>%
  select(PLAYER,
         position,
         situation,
         contains("onIce_F")) %>%
...
nhl_onIceAStats  <- salaryMPPlayer15 %>%
  select(PLAYER,
         position,
         situation,
         contains("onIce_A")) %>%
...
salaryMPPlayer15_neededVariables <- salaryMPPlayer15 %>%
...
salaryMPPlayer15_neededVariables <- salaryMPPlayer15_neededVariables %>%
  pivot_wider(id_cols = c(PLAYER, 
                          position,
                          team),
...
finalData_salaryMPPlayer15 <- nhl_diff_table %>%
  inner_join(nhl_onIce_diff_table,
             by = c("PLAYER",
                    "position")) %>%
  inner_join(salaryMPPlayer15_neededVariables,
             by = c("PLAYER",
                    "position")) %>%
...
salaryFastRplayer15 <- read_csv("RawData/player_box_2016_FastR.csv")

# Select only needed variables
salaryFastRplayer15 <- salaryFastRplayer15 %>%
...
salaryFastRplayer15 <- salaryFastRplayer15[!duplicated(salaryFastRplayer15[c("PLAYER", "position", "game_id")]), ]
...
finalData_salary15 <- salaryCF15 %>%
  inner_join(salaryFastRplayer15,
             by = c("PLAYER",
                    "position")) %>%
  inner_join(finalData_salaryMPPlayer15,
             by = c("PLAYER",
                    "position")) %>%
  mutate(season = "2015-16")

finalData_salary15 <- finalData_salary15 %>%
  select(PLAYER,
         position,
         team,
         season,
...
library(janitor)
finalData_salary15 <- clean_names(finalData_salary15) %>%
  rename(plusMinus = plus_minus) %>%
  mutate(total_cap = 71400000,
...
write_csv(finalData_salary15, "UsedDataForProject/NHL player Stats and Salary 2015-16.csv")

write_rds(finalData_salary15, "UsedDataForProject/NHL player Stats and Salary 2015-16.rds")
